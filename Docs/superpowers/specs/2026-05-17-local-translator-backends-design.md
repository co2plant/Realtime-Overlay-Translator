...
앞으로는 Papago뿐 아니라 로컬 번역 모델, 테스트용 번역기, 다른 번역 엔진을 붙일 수 있어야 한다. 1차 목표는 실제 로컬 모델을 바로 확정하는 것이 아니라, 번역 기능을 교체 가능한 구조로 만드는 것이다.
...
1차 범위에서 실제 CTranslate2, Argos Translate, NLLB, Ollama 같은 모델 런타임은 붙이지 않는다. 실제 모델 연결은 인터페이스가 안정된 뒤 2차 범위에서 진행한다.
